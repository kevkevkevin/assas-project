...
import { ShieldCheck, CheckCircle, XCircle, Eye, FileText, CarFront, Banknote, ArrowRightLeft, X, MessageCircle, Phone, Mail, Clock, AlertCircle, Wrench, CheckSquare } from "lucide-react";
...
export default function AdminPanel() {
  ...
  const renderVehicleInfo = (item: any) => {
    switch (activeTab) {
      case 'leases': return <><p className="font-bold text-slate-700">{item.make} {item.model} ({item.year})</p><p className="text-xs text-slate-500">{t("payment")}: {item.monthlyPayment} {t("perMonth")}</p></>;
      case 'rentals': return <><p className="font-bold text-slate-700">{item.make} {item.model} ({item.year})</p><p className="text-xs text-slate-500">{t("price")}: {item.dailyPrice} {t("perDay")}</p></>;
      case 'finance': return <><p className="font-bold text-slate-700">{item.carModel}</p><p className="text-xs text-slate-500">{t("estPayment")}: {item.estimatedMonthly || item.price} {t("sar")}</p></>;
      case 'swaps': return <><p className="font-bold text-slate-700">{t("has")}: {item.myMake} {item.myModel}</p><p className="text-xs text-purple-600 font-medium mt-0.5">{t("wants")}: {item.targetMake || 'Any'} {item.targetType || 'Any'}</p></>;
  ...
  return (
    <div className="max-w-7xl mx-auto pb-10 relative text-start">
      ...
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-900">{t("adminControlPanel")}</h1>
          <p className="text-slate-500 mt-1">{t("adminSubtitle")}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 bg-slate-100 p-1.5 rounded-2xl w-fit mb-8">
        ...
        <button onClick={() => setActiveTab('swaps')} className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'swaps' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}>
          <ArrowRightLeft size={16} /> {t("tabSwap")}
        </button>
            ...
            <table className="w-full text-start border-collapse min-w-[800px]">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100 text-xs uppercase tracking-wider text-slate-500">
                  <th className="p-5 font-bold">{t("tableUser")}</th>
                  <th className="p-5 font-bold">{t("tableRequestDetails")}</th>
                  <th className="p-5 font-bold">{t("tableDate")}</th>
                  <th className="p-5 font-bold">{t("tableStatus")}</th>
                  <th className="p-5 font-bold text-end">{t("tableActions")}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50 text-sm">
                {data.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="p-5">
                      <p className="font-bold text-slate-900">{item.userName || t("unknownUser")}</p>
                    ...
                    <td className="p-5 text-slate-500 text-xs font-medium">
                      {item.createdAt?.seconds ? new Date(item.createdAt.seconds * 1000).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }) : t("justNow")}
                    </td>
                    <td className="p-5">
                      <span className={`px-3 py-1.5 rounded-lg text-xs font-bold border ${
                        ...
                        {item.status || t("underReview")}
                      </span>
                    </td>
                    <td className="p-5 text-end flex justify-end gap-2">
      ...
      {viewingItem && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl p-8 w-full max-w-2xl shadow-2xl relative animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <button onClick={() => setViewingItem(null)} className="absolute top-6 right-6 rtl:left-6 rtl:right-auto text-slate-400 hover:text-slate-900 bg-slate-50 hover:bg-slate-100 p-2 rounded-full transition-colors">
              <X size={20} />
            </button>

            <h2 className="text-2xl font-bold text-slate-900 mb-2 capitalize">{activeTab === 'maintenance' ? t("serviceRequestDetails") : `${activeTab.slice(0, -1)} ${t("requestDetailsModal")}`}</h2>
            <p className="text-slate-500 text-sm mb-6 pb-6 border-b border-slate-100">
              {t("submittedBy")} <span className="font-bold text-slate-700">{viewingItem.userName}</span>
            </p>

            {activeTab !== 'maintenance' && (
              <div className="mb-8 pb-6 border-b border-slate-100">
                <h3 className="text-sm font-bold text-slate-900 mb-4 flex items-center gap-2">
                  <ShieldCheck className="text-primary" size={18} /> {t("dlVerification")}
                </h3>
                ...
                {!licenseChecked && !licenseLoading ? (
                  ...
                  >
                    <FileText size={18} /> {t("checkDL")}
                  </button>
                ) : licenseLoading ? (
                  <div className="text-sm text-slate-500 flex items-center gap-2 font-medium">
                    <Clock className="animate-spin text-primary" size={18} /> {t("fetchingFile")}
                  </div>
                ) : userLicense ? (
                  <div className="animate-in fade-in slide-in-from-top-2">
                    <span className="inline-block bg-green-50 text-green-600 px-3 py-1 rounded-lg text-xs font-bold mb-3 border border-green-100">
                      <CheckCircle size={14} className="inline ltr:mr-1 rtl:ml-1" /> {t("verifiedOnFile")}
                    </span>
                  ...
                  <div className="text-sm text-slate-500 bg-slate-50 p-4 rounded-xl border border-slate-100 flex items-center gap-2">
                     <AlertCircle size={18} className="text-slate-400" /> {t("noDLFound")}
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 'maintenance' && viewingItem.images && viewingItem.images.length > 0 && (
              <div className="mb-6">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">{t("uploadedPhotos")}</p>
            ...
            {activeTab === 'finance' && viewingItem.status === 'Approved' && (
              <div className="mb-8 p-6 bg-slate-50 rounded-2xl border border-slate-200">
                <h3 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
                  <Banknote className="text-primary" size={20} /> {t("financialOverview")}
                </h3>
                
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">{t("outstandingBalance")} ({t("sar")})</label>
                    <div className="flex">
                      <input type="number" id="balanceAdmin" defaultValue={viewingItem.outstandingBalance} className="w-full h-10 px-3 rounded-l-lg rtl:rounded-l-none rtl:rounded-r-lg border border-slate-300 focus:outline-none" />
                      ...
                      }} className="bg-primary text-white px-3 text-sm font-bold rounded-r-lg rtl:rounded-r-none rtl:rounded-l-lg">{t("saveBtn")}</button>
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">{t("totalPaid")} ({t("sar")})</label>
                    <div className="flex">
                      <input type="number" id="paidAdmin" defaultValue={viewingItem.totalPaid} className="w-full h-10 px-3 rounded-l-lg rtl:rounded-l-none rtl:rounded-r-lg border border-slate-300 focus:outline-none" />
                      <button onClick={async () => {
                      ...
                      }} className="bg-green-600 hover:bg-green-700 text-white px-3 text-sm font-bold rounded-r-lg rtl:rounded-r-none rtl:rounded-l-lg">{t("saveBtn")}</button>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6 pb-6 border-b border-slate-200">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">{t("monthsPaid")}</label>
                    <div className="flex">
                      <input type="number" id="monthsPaidAdmin" defaultValue={viewingItem.monthsPaid} className="w-full h-10 px-3 rounded-l-lg rtl:rounded-l-none rtl:rounded-r-lg border border-slate-300 focus:outline-none" />
                      <button onClick={async () => {
                      ...
                      }} className="bg-slate-700 text-white px-3 text-sm font-bold rounded-r-lg rtl:rounded-r-none rtl:rounded-l-lg">{t("saveBtn")}</button>
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">{t("remainingMonths")}</label>
                    <div className="flex">
                      <input type="number" id="remMonthsAdmin" defaultValue={viewingItem.remainingMonths} className="w-full h-10 px-3 rounded-l-lg rtl:rounded-l-none rtl:rounded-r-lg border border-slate-300 focus:outline-none" />
                      <button onClick={async () => {
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{t("uploadNewReceipt")}</label>
            ...
            <div className="flex gap-4 pt-6 border-t border-slate-100">
              <button onClick={() => updateStatus(viewingItem, 'Approved')} className="flex-1 py-3 bg-green-50 hover:bg-green-100 text-green-700 rounded-xl font-bold transition-colors flex items-center justify-center gap-2">
                <CheckCircle size={18} /> {activeTab === 'maintenance' ? t("approveService") : t("approveBtn")}
              </button>
