      ...
      <header className="fixed top-6 w-full z-50 flex justify-center px-4 pointer-events-none">
        <div className="w-full max-w-[1600px] bg-white/85 backdrop-blur-md border border-slate-200 shadow-xl shadow-slate-200/40 h-20 rounded-full flex items-center justify-between px-6 md:px-8 pointer-events-auto transition-all">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            {/* 3. NEW LANGUAGE TOGGLE BUTTON */}
            <button 
              onClick={toggleLanguage} 
              className="font-bold text-slate-500 hover:text-primary transition-colors px-2 md:px-4 hidden sm:block"
            >
            ...
            {loading ? (
              ...
              <Link href="/dashboard" className="flex items-center gap-3 bg-white border border-slate-200 hover:border-primary/40 px-2 py-1.5 pr-5 rounded-full transition-all hover:shadow-md shadow-sm group">
                ...
                <span className="text-sm font-extrabold text-primary flex items-center gap-1">{t("dashboard")} <ArrowRight size={14} /></span>
              </Link>
            ) : (
              <div className="flex items-center gap-2 sm:gap-4">
                <Link href="/login" className="font-bold text-slate-500 hover:text-slate-900 transition-colors hidden sm:block px-2">{t("login")}</Link>
                <Link href="/signup" className="bg-slate-900 hover:bg-slate-800 text-white px-6 md:px-8 py-3 rounded-full font-bold shadow-lg transition-all hover:shadow-xl hover:-translate-y-0.5 text-sm md:text-base">{t("signup")}</Link>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          ...
          <div className="">
            ...
            <h1 className="text-5xl md:text-6xl xl:text-7xl font-extrabold text-slate-900 tracking-tight mb-6 leading-[1.1] animate-in fade-in slide-in-from-bottom-6 duration-700">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-400">{t("heroHighlight")}</span>
            </h1>
            ...
            <form onSubmit={handleSearch} className="bg-white p-2 rounded-full shadow-2xl shadow-slate-200/60 border border-slate-100 flex items-center max-w-xl mb-12 animate-in fade-in zoom-in-95 duration-1000 delay-150 relative">
              ...
              />
              <button type="submit" className="bg-primary hover:bg-secondary text-white px-6 md:px-8 py-3.5 rounded-full font-bold transition-all shadow-md shadow-blue-500/20 hover:shadow-lg flex items-center gap-2">
                <span className="hidden md:block">{t("searchButton")}</span>
                <ArrowRight size={20} className="md:hidden rtl:rotate-180" />
              </button>
            </form>
            ...
            <div className="flex flex-wrap items-center gap-8 opacity-60 grayscale transition-all hover:grayscale-0 hover:opacity-100 duration-500">
              <div className="flex items-center gap-2 font-bold text-slate-700"><ShieldCheck className="text-green-500" /> {t("securePayments")}</div>
              <div className="flex items-center gap-2 font-bold text-slate-700"><Zap className="text-yellow-500" /> {t("instantApprovals")}</div>
              <div className="flex items-center gap-2 font-bold text-slate-700"><Star className="text-primary" /> {t("verifiedUsers")}</div>
            </div>
          </div>
          ...
          <div className="relative w-full h-[400px] lg:h-[600px] animate-in fade-in slide-in-from-right-8 duration-1000">
            ...
            <div className="absolute -left-4 top-1/4 bg-white/90 backdrop-blur-md px-6 py-3 rounded-2xl shadow-xl border border-slate-100 flex items-center gap-3 animate-bounce shadow-slate-200">
               ...
               <div>
                 <p className="text-xs font-bold text-slate-400 uppercase">{t("saleNow")}</p>
            ...
            >
              ...
              <div className="p-6 flex flex-col flex-1 justify-between">
                ...
                <button 
                  ...
                  className="w-full bg-[#121E5B] hover:bg-[#0B4FB6] text-white font-bold py-3 rounded-xl transition-colors"
                >
                  {t("bookNow")}
                </button>
              </div>
              ...
              <div className="p-6 flex flex-col flex-1 justify-between">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex-1">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">{t("offering")}</p>
                  ...
                  <div className="flex-1 text-right">
                    <p className="text-xs font-bold text-purple-400 uppercase tracking-wider mb-1">{t("lookingFor")}</p>
